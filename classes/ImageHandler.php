	...
	class ImageHandler{
		...
		function uploadImage(){
			...
			if(strpos(strtolower(PHP_OS),"win")!==false){
				$realName=iconv('utf-8','gbk',$realName);//Linux系统中不用转码，linux中使用utf-8，不使用gbk
			}				
			...
			if(filesize($fileName)<200000){
				...
				if(isset($_SESSION['username'])){
					...
					if(strpos(strtolower(PHP_OS),"win")!==false){
						$username=iconv('utf-8','gbk',$username);//Linux系统中不用转码
					}	
					...
					if(strpos(strtolower(PHP_OS),"win")!==false){
						$newPath=iconv('gbk','utf-8',$newPath);//Linux系统中不用转码	
					}
